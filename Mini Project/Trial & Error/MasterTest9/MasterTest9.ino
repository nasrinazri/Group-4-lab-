...
#define FIX_STOP_BUTTON_PIN 3  // Pin for Fix Stop button (new button)
...
volatile bool fixStopFlag = false;  // Flag for Fix Stop button press
volatile bool stopFlag = false; // Global flag to detect button press
bool fixLoopActive = false; // Flag to control the fix loop
...
void setup() {
    ...
    pinMode(FIX_STOP_BUTTON_PIN, INPUT_PULLUP); // Fix stop button
    attachInterrupt(digitalPinToInterrupt(STOP_BUTTON_PIN), stopButtonISR, FALLING); // Interrupt on button press
    attachInterrupt(digitalPinToInterrupt(FIX_STOP_BUTTON_PIN), fixStopButtonISR, FALLING); // Interrupt for fix stop
...
void loop() {
    ...
    if (!isCardAuthenticated) {
        ...
        // Handle commands from Serial or Serial1
        if (Serial.available() > 0 || Serial1.available() > 0) {
            ...
            if (command.startsWith("ADD")) {
                ...
                fixLoopActive = !fixLoopActive;
                if (fixLoopActive) {
                    Serial.println("Starting continuous temperature monitoring...");
                } else {
                    Serial.println("Stopping continuous temperature monitoring...");
                }
            } else {
    ...
    // Run the fix function only if continuous monitoring is active
    if (fixLoopActive) {
        fix(); // Call the fix function periodically
        delay(2000); // Delay between temperature readings to prevent overloading the loop
    }
}
...
// ISR for the Fix Stop Button (FIX_STOP_BUTTON_PIN)
void fixStopButtonISR() {
    static unsigned long lastInterruptTime = 0;
    unsigned long interruptTime = millis();

    // Debounce check (ignore if within 200ms of the last interrupt)
    if (interruptTime - lastInterruptTime > 200) {
        fixStopFlag = true;  // Set the flag to stop the fix function
        logToBT("Fix stop activated.");
    }
    lastInterruptTime = interruptTime;
}
...
void fix() {
    // Check if the Fix Stop button was pressed
    if (fixStopFlag) {
        logToBT("Fix operation stopped by user.");
        indicateEmergencyStop();  // Update LED indicators (show stop status)
        fixStopFlag = false; // Reset the flag for future operations
        return; // Exit the fix() function immediately without reading temperature
    }

    // Continue with the fix operation if Fix Stop wasn't pressed
